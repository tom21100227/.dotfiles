#!/bin/bash
# macOS-specific setup. Invoke via the top-level setup.sh (which exports
# DOTFILES_DIR). Installs Homebrew + Brewfile, sets macOS defaults, and
# delegates shared steps (zdotdir, stow, chsh) to scripts/common.sh.

: "${DOTFILES_DIR:?DOTFILES_DIR must be set (run via setup.sh)}"

ITERM_SETTINGS_DIR="$HOME/.config/iterm2"
ISTAT_MENUS_SETTINGS="$DOTFILES_DIR/istatmenus/iStat Menus Settings.ismp7"
...
# --- Shared steps (zdotdir, .zshenv, stow, chsh) --------------------------
export ZSH_BIN="/opt/homebrew/bin/zsh"
